...
CREATE TABLE IF NOT EXISTS `NameNotFoundDB`.`sheet` (
  ...
  `sheet_content` LONGTEXT NOT NULL,
  `sheet_image` VARBINARY(255) NULL,
...
-- -----------------------------------------------------
-- Table `NameNotFoundDB`.`user`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `NameNotFoundDB`.`user` (
...
CREATE TABLE IF NOT EXISTS `NameNotFoundDB`.`user_has_request` (
  ...
  CONSTRAINT `fk_user_has_request_user`
    ...
    REFERENCES `NameNotFoundDB`.`user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
